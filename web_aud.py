...
from b2_storage import upload_to_b2
...
def store_audio_file(audio_file, audio_filename):
    """Process audio and upload to Backblaze B2."""
    try:
        logging.info(f"🎵 Starting audio processing for: {audio_filename}")
        ...
        logging.info(f"💾 WAV conversion complete: {len(wav_data)} bytes")
        
        # Upload to Backblaze B2
        b2_url = upload_to_b2(wav_data, audio_filename)
        logging.info(f"✅ Audio uploaded to Backblaze B2: {b2_url}")
...
def index():
    ...
    try:
        if request.method == "POST":
            ...
            logging.info(f"🎯 Processing audio upload - File: {audio_filename}")

            try:
                # Read audio data for background processing ONLY
                audio_data = audio_file.read()
                logging.info(f"📊 Audio data size: {len(audio_data)} bytes")
                
                # DON'T process here - let background thread handle everything
                audio_url = None  # Background thread will handle the upload
            except Exception as e:
            ...
            try:
                thread = threading.Thread(
                    ...
                    args=(form_chunk_id, audio_data, audio_filename, name),
...
def update_chunk_in_background_thread(chunk_id, audio_data, audio_filename, name):
    ...
    try:
        ...
        logging.info(f"📁 Processing audio: {audio_filename}")
        ...
        try:
            ...
            logging.info(f"💾 Processed WAV data: {len(wav_data)} bytes")
            
            # Upload to Backblaze B2
            audio_url = upload_to_b2(wav_data, audio_filename)
            logging.info(f"✅ Audio uploaded to B2: {audio_url}")
            
        except Exception as upload_error:
            logging.error(f"❌ Audio processing/upload failed: {upload_error}", exc_info=True)
            raise
        ...
        with conn.cursor() as cursor:
            ...
            values = ("no", "done", audio_url, name, datetime.now(), chunk_len, chunk_id)
            ...
            if rows_affected > 0:
                logging.info(f"✅ Chunk {chunk_id} updated successfully in database")
            else:
